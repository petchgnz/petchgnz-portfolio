...
const EMPLOYMENT_TYPES: EmploymentType[] = ['FULL_TIME', 'PART_TIME', 'FREELANCE', 'CONTRACT']
...
export function ExperienceEditSheet({ open, onOpenChange, defaultValues }: Props) {
  ...
  const getDefaults = (): FormValues => ({
    ...
    employmentType: (defaultValues?.employmentType as EmploymentType) ?? 'FULL_TIME',
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="overflow-y-auto">
        <SheetHeader>
          <SheetTitle>{isEditing ? 'Edit Experience' : 'Add Experience'}</SheetTitle>
        </SheetHeader>
              ...
              render={({ field, fieldState }) => (
                <Field data-invalid={fieldState.invalid}>
                  <FieldLabel htmlFor={name}>
                    {name === 'startDate' ? 'Start Date' : 'End Date (leave empty if current)'}
                  </FieldLabel>
                  <Input
                    {...field}
                    id={name}
